...
pub fn exec_goat<CTX: ContextTr>(context: &mut CTX) -> Result<(), <CTX::Db as Database>::Error> {
    ...
    if let Some(deposit) = context.tx().deposit() {
        ...
        let mut deposit_account = context.journal_mut().load_account_mut(deposit.address)?;
        let balance = deposit_account
            .data
            .info
            .balance
            .saturating_add(deposit.amount);
        deposit_account.set_balance(balance);

        // Add the tax to GF.
        if deposit.tax > U256::ZERO {
            let mut foundation_account = context
                .journal_mut()
                .load_account_mut(GOAT_FOUNDDATION_CONTRACT)?;
            let balance = foundation_account
                .data
                .info
                .balance
                .saturating_add(deposit.tax);
            foundation_account.set_balance(balance);
        }
    }
    ...
    if let Some(withdraw) = context.tx().withdraw() {
        ...
        if withdraw.amount > U256::ZERO {
            context.journal_mut().transfer(
                GOAT_LOCKING_CONTRACT,
                withdraw.address,
                withdraw.amount,
            )?;
        }
